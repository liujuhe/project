...
<script type="text/javascript">
	...
	*/
	...
	function changeIcon(stage,i){
		...
		//如果当前阶段的可能性不为0，前七个有可能是绿圈，绿标，黑圈，后两个一定是黑叉
		}else{
			//遍历前七个
			for (var i=0;i<point ;i++){
				//如果是当前阶段
		...
		<%
			...
			if ("0".equals(currentPossibility)){
				...
				%>
					...
					<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
						  class="glyphicon glyphicon-remove mystage"
						data-toggle="popover" data-placement="bottom"
						data-content="<%=dv.getText()%>" style="color: #000;"></span>
					-----------

				<%
						}

					//如果遍历出来的阶段不为0，说明是前七个，一定是黑圈
					}else{
						//黑圈
				%>
			...
			//如果当前阶段的可能性不为0 前7个有可能性是绿圈，绿色标记，黑圈，后两个一定是黑叉
				...
				<%
					...
					//如果遍历出来的阶段可能性不为0 说明前7个阶段 绿圈，绿色标记，黑圈
					}else{
						//如果是当前阶段
						if (i==index){
