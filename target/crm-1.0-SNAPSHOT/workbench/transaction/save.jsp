...
<script type="text/javascript" src="jquery/bootstrap-datetimepicker-master/locale/bootstrap-datetimepicker.zh-CN.js"></script>
	...
	<script type="text/javascript">
		...
		* 		stage2ToPossibility.properties
		* 		01资质审查=10
		* 		02需求分析=25
		* 		...
		*
		* 		stage2ToPossibility.properties这个文件表示的是阶段和键值对之间的对应关系
		* 		将来，我们通过stage，以及对应关系，来取得可能性这个值
		* 		这种需求在交易模块中需要大量的使用到
		*
		* 		所以我们就需要将该文件解析在服务器缓存中
		* 		application.setAttribute("stage2ToPossibility.properties")
		* */
		$(function (){
			...
			$("#create-stage").change(function (){
				...
				*
				* 		阶段有了stage
				* 		阶段可能性之间的对应关系pMap，但是pMap是java语言中的键值对关系（java中的map对象）
			...
			//为“放大镜”图标，绑定事件，打开查找联系人的模态窗口
			$("#contactsName").click(function (){
				contactLoadDate();
				$("#findContacts").modal("show");
			});

			//为 提交（联系人）按钮绑定事件，填充市场活动源（填写两项信息 名字+id）
			$("#submitContactBtn").click(function (){
				var $xz =$("input[name=contactId]:checked");
				var id = $xz.val();
				var name = $("#"+id).html();
		...
		//局部刷新联系人数据
		function contactLoadDate(){
			$.ajax({
				url:"workbench/transaction/selectContact.do",
				type:"get",
				dataType:"json",
				data:{
				...
				success:function (data){
					...
					$.each(data,function (i,n){
						html += '<tr>'
						html += '<td><input type="radio" name="contactId" value="'+n.id+'"/></td>'
...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="contactName" class="form-control" style="width: 300px;" placeholder="请输入联系人名称，支持模糊查询">
						    <span class="glyphicon glyphicon-search form-control-feedback"></span>
						  </div>
						</form>
					</div>
	...
	<form action="workbench/transaction/save.do" id="tranForm" method="post" class="form-horizontal" role="form" style="position: relative; top: -30px;">
		<input type="hidden" name="activityId" id="hidden-activityId">
		<input type="hidden" name="contactsId" id="hidden-contactId">
		...
		<div class="form-group">
			...
			<label for="create-activitySrc" class="col-sm-2 control-label">市场活动源&nbsp;&nbsp;<a href="javascript:void(0);" ><span class="glyphicon glyphicon-search" id="activitySource"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
		...
		<div class="form-group">
			<label for="create-contactsName" class="col-sm-2 control-label">联系人名称&nbsp;&nbsp;<a href="javascript:void(0);" ><span class="glyphicon glyphicon-search" id="contactsName"></span></a></label>
			<div class="col-sm-10" style="width: 300px;">
